...
export function renderSummary(
  ...
  return html`
    <div class="space-y-3">
      ...
      <div>
        <div
          class="text-[10px] font-medium mb-1 text-gray-600 dark:text-gray-400 uppercase tracking-wide"
        >
          Monthly Total
        </div>
        <div class="flex items-baseline gap-2 flex-wrap">
          <span class="text-lg font-bold ${totalHoursColor}">${totalMonthlyHours.toFixed(1)}h</span>
          <span class="text-xs text-gray-500 dark:text-gray-400"
            >/ ${totalRequiredHours.toFixed(1)}h</span
          >
      ...
      <div class="space-y-1.5 pt-1 border-t border-gray-200 dark:border-gray-700">
        <div class="flex items-baseline justify-between gap-2 flex-wrap">
          <span
            class="text-[10px] font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wide"
          >
            Workday
          </span>
          <div class="flex items-baseline gap-1.5 flex-wrap">
            <span class="text-xs font-semibold ${getHoursColor(workdayHours, requiredWorkdayHours)}"
              >${workdayHours.toFixed(1)}h</span
            >
            <span class="text-[10px] text-gray-500 dark:text-gray-400"
              >/ ${requiredWorkdayHours.toFixed(1)}h</span
            >
        ...
        <div class="flex items-baseline justify-between gap-2 flex-wrap">
          ...
          <div class="flex items-baseline gap-1.5 flex-wrap">
            <span
              ...
              >${publicHolidayHours.toFixed(1)}h</span
            >
            <span class="text-[10px] text-gray-500 dark:text-gray-400"
              >/ ${requiredPublicHolidayHours.toFixed(1)}h</span
            >
            ...
            ${publicHolidayHasWarning && publicHolidayOvertimeHours > 0
              ? html`<span
                  ...
                  >⚠ +${publicHolidayOvertimeHours.toFixed(1)}h</span
                >`
              : ""}
          </div>
        </div>
      ...
      <div class="pt-1 border-t border-gray-200 dark:border-gray-700">
        ...
        <div class="flex items-baseline gap-2 flex-wrap">
          <span class="text-sm font-bold ${overtimeColor}">${overtimeHours.toFixed(1)}h</span>
          ${overtimeHours > 0
            ? html`<span
              ...
              >`
            : html`<span
                class="inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-medium bg-gray-100 dark:bg-gray-900/30 text-gray-600 dark:text-gray-400"
                >No overtime</span
              >`}
        </div>
      </div>
