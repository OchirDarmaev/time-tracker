...
export async function handleManagerReportsRoutes(
  action: 'index' | 'worker' | 'project',
  context: any
) {
  ...
  if (action === 'worker') {
    ...
    if (!userId) {
      return { status: 200 as const, body: { body: '<p class="text-gray-500">Select a worker to view reports.</p>' } };
    }
  ...
  if (action === 'project') {
    ...
    if (!projectId) {
      return { status: 200 as const, body: { body: '<p class="text-gray-500">Select a project to view reports.</p>' } };
    }
