...
export const projectUserModel = {
  ...
  getByUserId(userId: number): ProjectUser[] {
    return db.prepare('SELECT * FROM project_users WHERE user_id = ? AND suppressed = 0').all(userId) as ProjectUser[];
  },

  create(userId: number, projectId: number): ProjectUser {
    const result = db.prepare('INSERT INTO project_users (user_id, project_id, suppressed) VALUES (?, ?, 0)').run(userId, projectId);
    return db.prepare('SELECT * FROM project_users WHERE id = ?').get(result.lastInsertRowid as number) as ProjectUser;
  },
  ...
  toggleSuppress(id: number): ProjectUser | undefined {
    ...
    return db.prepare('SELECT * FROM project_users WHERE id = ?').get(id) as ProjectUser | undefined;
  },
};
